...
for r in results:
    if r.boxes:
        # 각 객체의 바운딩 박스를 이미지에 그립니다.
        for box in r.boxes.xyxy:
            x1, y1, x2, y2 = map(int, box.tolist())
            cv2.rectangle(img, (x1, y1), (x2, y2), (255, 0, 0), 2)
            
# 이미지를 RGB에서 BGR로 변환하여 OpenCV로 출력 가능한 형태로 변경합니다.
img = cv2.cvtColor(img, cv2.COLOR_RGB2BGR)

# 바운딩 박스가 그려진 이미지를 화면에 표시하고, 아무 키나 누를 때까지 대기합니다.
cv2.imshow("Bounding Box", img)
cv2.waitKey(0)
cv2.destroyAllWindows()

# 실행방법 
# python hw2_2.py city1.jpg  OR  python hw2_2.py city2.jpg
